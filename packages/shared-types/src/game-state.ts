...
export type GameAction = 
  | { type: 'JOIN_GAME'; playerId: string; playerName: string; position: PlayerPosition }
  ...
  | { type: 'COMPLETE_TRICK'; trickId: string }
  | { type: 'COMPLETE_HAND'; handScore: HandScore }
  | { type: 'START_NEW_HAND'; dealer: string }
  | { type: 'END_GAME'; winner: PartnershipTeam }
  | { type: 'RESET_GAME' };
 ...
 */
export function createEmptyPartnershipState(): PartnershipState {
  return {
    northSouth: {
      players: ['', ''],
      currentHandScore: 0,
